...
const SOCIAL_ICONS = {
  ...
  telegram: '<svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18"><path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0a12 12 0 00-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>',
...
const SOCIAL_LABELS = {
  linkedin: 'LinkedIn URL', facebook: 'Facebook URL', instagram: 'Instagram URL',
  twitter: 'Twitter/X URL', website: 'Website URL', youtube: 'YouTube URL',
  telegram: 'Telegram Username', discord: 'Discord Username',
};

let contact = null;
let isEditing = false;
let snapshot = null;
const maskedFields = new Set(['pan', 'aadhaar', 'nationalIdNumber']);
...
function field(name, label, opts = {}) {
  const {
    type = 'text', validate = '', value = '', readonly = false,
    as = 'input', options = [], rows = 3, masked = false, id = name,
    alwaysReadonly = false,
  } = opts;
  const locked = alwaysReadonly || readonly || !isEditing;
  const ro = locked && as !== 'select' ? 'readonly' : '';
  const dis = locked ? 'disabled' : '';
  const displayVal = masked ? getMaskedDisplay(id, value) : value;
  const maskedClass = masked && maskedFields.has(id) ? 'field__input--masked' : '';
    ...
    return `<div class="field note-field" data-note-field>
      <div class="note-field__head">
        ...
        <div class="note-field__tools">
          ...
          <button type="button" class="note-field__expand" data-expand-note="${id}" title="Expand note">
            <i data-lucide="maximize-2"></i>
          </button>
        </div>
      </div>
      <textarea class="field__textarea field__textarea--collapsed" name="${name}" id="${id}" rows="${rows}"
        maxlength="500" data-validate="${validate}" data-note-input ${ro}>${esc(value)}</textarea>${err}</div>`;
  }
  return `<div class="field"><label class="field__label">${label}</label>
    <input class="field__input ${maskedClass}" type="${type}" name="${name}" id="${id}"
      value="${esc(displayVal)}" data-validate="${validate}"
      ${masked ? `data-masked="true" data-mask-type="${id}" data-real-value="${esc(value)}"` : ''} ${ro} />${err}</div>`;
}
...
function fieldPhone(name, label, value, validate = '') {
  ...
  return `<div class="field"><label class="field__label">${label}</label>
    <div class="phone-input">
      ...
      <input class="field__input phone-input__field" type="tel" name="${name}" id="${name}"
        value="${esc(display)}" data-validate="${validate}" data-phone ${ro} />
    </div>
...
function fieldMasked(id, label, value, maskType, validate = '') {
  ...
  const ro = !isEditing ? 'readonly' : '';
  return `<div class="field field--masked-wrap">
    <label class="field__label">${label}</label>
    <div class="masked-input">
      <input class="field__input ${maskedFields.has(id) ? 'field__input--masked' : ''}" type="text"
        name="${id}" id="${id}" value="${esc(display)}" data-validate="${validate}"
        data-masked="true" data-mask-type="${maskType}" data-real-value="${esc(value)}" ${ro} />
      ${!isEditing ? `<button type="button" class="masked-input__toggle" data-reveal="${id}" title="Reveal">
...
function addressFields(formKey) {
  const sourceKey =
    formKey === 'permanentAddress' && contact.permanentAddressSameAsCurrent
      ? 'currentAddress' : formKey;
  const addr = contact[sourceKey];
  const ro = !isEditing || (formKey === 'permanentAddress' && contact.permanentAddressSameAsCurrent);
  const fields = ['line1', 'line2', 'line3', 'city', 'state', 'country', 'zipCode'];
  const labels = ['Address Line 1', 'Address Line 2', 'Address Line 3', 'City', 'State', 'Country', 'Zip Code'];
  return fields.map((f, i) => {
    const validate = f === 'zipCode' ? 'zip' : f === 'line1' || f === 'city' ? 'required' : '';
    return field(`${formKey}.${f}`, labels[i], {
      value: addr[f], readonly: ro, validate, id: `${formKey}_${f}`,
    });
  }).join('');
}
...
function collapsibleCard(sectionId, sectionLetter, icon, title, desc, body, open = true) {
  ...
  return `<div class="card ${open ? 'is-open' : ''}" id="section-${sectionId}"
    data-section="${sectionLetter}" data-card="${sectionId}" style="--accent:${accent}">
...
function renderStickyHeader() {
  ...
  document.getElementById('stickyHeader').innerHTML = `
    <div class="contact-sticky__inner" id="section-a" data-section="A">
      <div class="contact-sticky__profile">
        <div class="avatar-wrap">
          <div class="avatar avatar--lg" id="avatarDisplay">${avatarContent}</div>
          ${isEditing ? `<label class="avatar-upload" title="Upload profile picture">
        ...
        <div>
          ...
          <div class="contact-sticky__meta">
            ...
            <span class="badge badge--source">${esc(contact.sourceChannel)}</span>
          </div>
        </div>
      </div>
      <div class="contact-sticky__actions">
        ...
        <a href="mailto:${esc(contact.emailAddress || '')}" class="btn btn--outline btn--action"><span class="btn-emoji">📧</span><span class="hide-mobile">Send Email</span></a>
...
function renderSections() {
  ...
  const personal = collapsibleCard('personal', 'B', 'user', 'Personal Information', 'Demographics and identity', `
    <div class="card-grid">
  ...
  const contactInfo = collapsibleCard('contact-info', 'C', 'phone', 'Contact Information', 'Phone numbers and emails', `
    <div class="card-grid card-grid--2">
      ${fieldPhone('mobileNumber', 'Mobile Number', contact.mobileNumber, 'required|phone')}
      ${fieldPhone('alternateMobileNumber', 'Alternate Mobile Number', contact.alternateMobileNumber, 'phone')}
      ${field('emailAddress', 'Email Address', { type: 'email', value: contact.emailAddress, validate: 'required|email' })}
      ${field('alternateEmailAddress', 'Alternate Email Address', { type: 'email', value: contact.alternateEmailAddress, validate: 'email' })}
    </div>`);

  const address = collapsibleCard('address', 'D', 'map-pin', 'Address Information', 'Current and permanent addresses', `
      ...
      <div class="address-split__col">
        <div class="address-split__head">
          ...
          <label class="toggle-row toggle-row--compact">
            <span class="toggle-row__label">Same as Current Address</span>
            <div class="toggle">
  ...
  const professional = collapsibleCard('professional', 'E', 'briefcase', 'Professional Information', 'Work and expertise', `
    <div class="card-grid">
      ...
      ${field('experienceYears', 'Experience Years', { type: 'number', value: contact.experienceYears, validate: 'number:0:50' })}
      ${field('experienceMonths', 'Experience Months', { type: 'number', value: contact.experienceMonths, validate: 'number:0:11' })}
  ...
  const identification = collapsibleCard('identification', 'F', 'fingerprint', 'Identification', 'Government ID (masked)', `
    <div class="card-grid">
      ${fieldMasked('pan', 'PAN', contact.pan, 'pan', 'pan')}
      ${fieldMasked('aadhaar', 'Aadhaar', contact.aadhaar, 'aadhaar', 'aadhaar')}
      ${fieldMasked('nationalIdNumber', 'National Identification Number', contact.nationalIdNumber, 'default')}
    </div>`);

  const notes = collapsibleCard('notes', 'J', 'sticky-note', 'Notes', 'Expandable notes with character counter', `
    <div class="card-grid card-grid--notes">
      ${[1, 2, 3, 4, 5].map((n) => field(`note${n}`, `Note ${n}`, { as: 'textarea', value: contact[`note${n}`], rows: 5 })).join('')}
    </div>`);

  const engagement = collapsibleCard('engagement', 'G', 'activity', 'Engagement Dashboard', 'Click cards to toggle when editing', `
    <div class="engagement-grid">
      ${['talked', 'met', 'responded'].map((k) => {
        const yes = e[k];
        const label = k.charAt(0).toUpperCase() + k.slice(1);
        const clickable = isEditing ? 'engagement-card--clickable' : '';
        return `<button type="button" class="engagement-card engagement-card--${yes ? 'yes' : 'no'} ${clickable}"
          data-engagement-toggle="${k}" ${!isEditing ? 'disabled' : ''}>
          <div class="engagement-card__emoji">${ENGAGEMENT_EMOJI[k]}</div>
  ...
  const marketing = collapsibleCard('marketing', 'H', 'megaphone', 'Marketing Preferences', 'Communication consent', `
    <div class="marketing-list">
      ${[
        ['emailConsent', 'Email Consent', false],
        ['smsConsent', 'SMS Consent', false],
        ['whatsappConsent', 'WhatsApp Consent', false],
        ['callConsent', 'Call Consent', false],
        ['doNotDisturb', 'Do Not Disturb', true],
      ].map(([key, label, isDnd]) => `
  ...
  const socialInputs = Object.keys(SOCIAL_ICONS).map((key) => {
    ...
    return `<div class="field social-field">
      ...
      <input class="field__input" type="text" name="social.${key}" value="${esc(val)}"
        placeholder="${key === 'telegram' || key === 'discord' ? '@username' : 'https://...'}"
        ${!isEditing ? 'readonly' : ''} />
    </div>`;
  }).join('');

  const social = collapsibleCard('social', 'I', 'share-2', 'Social Media', 'Icon-based profile links', `
    <div class="card-grid card-grid--social">${socialInputs}</div>`);

  const audit = collapsibleCard('audit', 'K', 'clock', 'Audit Information', 'Read-only record history', `
  ...
  document.getElementById('mainSections').innerHTML =
    personal + contactInfo + address + professional + identification + notes;
...
function bindInteractions() {
  ...
  document.querySelectorAll('[data-expand-note]').forEach((btn) => {
    btn.onclick = () => {
      ...
      if (ta) {
        ta.classList.toggle('field__textarea--expanded');
        ta.classList.toggle('field__textarea--collapsed');
      }
    };
  });
    ...
    upload.onchange = (ev) => {
      ...
      reader.onload = (r) => {
        ...
        document.getElementById('avatarDisplay').innerHTML =
          `<img src="${r.target.result}" alt="Profile" />`;
      };
  ...
  document.getElementById('btnEdit') && (document.getElementById('btnEdit').onclick = () => {
    isEditing = true;
    maskedFields.clear();
    ['pan', 'aadhaar', 'nationalIdNumber'].forEach((f) => maskedFields.add(f));
    renderAll();
  });
  ...
  document.querySelectorAll('input[name^="marketing."]').forEach((cb) => {
    cb.onchange = () => {
      ...
      if (row && cb.name === 'marketing.doNotDisturb') {
        row.classList.toggle('toggle-row--danger', cb.checked);
      }
    };
  });
}
...
function collectFormData() {
  ...
  form.querySelectorAll('input[type=checkbox]').forEach((el) => {
    if (!el.name) return;
    const [parent, child] = el.name.split('.');
    if (parent === 'engagement') data.engagement[child] = el.checked;
    if (parent === 'marketing') data.marketingPreferences[child] = el.checked;
  });

  ['pan', 'aadhaar', 'nationalIdNumber'].forEach((f) => {
...
async function init() {
  ...
  if (isNew) {
    contact = {
      ...
      firstName: '', lastName: '', fullName: 'New Contact', gender: 'Male',
      dateOfBirth: '', maritalStatus: 'Single', nationality: 'Indian', language: 'English', religion: '',
      ...
      socialMedia: { linkedin: '', facebook: '', instagram: '', twitter: '', website: '', youtube: '', telegram: '', discord: '' },
  ...
  document.getElementById('contactForm').onsubmit = async (ev) => {
    ...
    try {
      if (contact._isNew) {
        // apiCreateContact already returns a fully frontend-shaped contact
        // (mapped from the backend response inside api.js) — no manual
        // normalization needed here.
        contact = await apiCreateContact(data);
        if (contact._dbId) {
          window.history.replaceState({}, '', `contact.html?id=${contact._dbId}`);
        }
      } else {
